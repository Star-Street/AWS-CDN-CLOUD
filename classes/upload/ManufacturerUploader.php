...
class ManufacturerUploader extends UploadService
{
    const LANG_ID = 1;
    ...
    protected function getAllImages($entityIds = null, bool $needRecreateImages): array {
        ...
        $this->generateImages($elements, _PS_MANU_IMG_DIR_, $needRecreateImages);

        foreach ($elements as $elem) {
            $entityId = (int) $elem["id_manufacturer"];
            $name = $this->textHealper->clearText($elem["name"]);
            $description = $this->textHealper->clearText($elem["short_description"]);
    ...
    public function generateImages(array $elements, string $path, bool $needRecreateImages): bool {
        $time = date("H:i");
        $imgTypes = ImageType::getImagesTypes("manufacturers");

        if (!empty($elements)) {
            if ($needRecreateImages) {
                foreach ($elements as $elem) {
                    $id = (int) $elem["id_manufacturer"];
                    $time = date("H:i");
                    $mainImg = $path . $id . ".jpg";
                    ...
                    if (!empty($imgThumb)) {
                        $this->logger->log("generate", "Time: {$time} | ID: $id | Target: Server | Status: 200 | Image: /img/tmp/manufacturer_$id.jpg");
                    } else {
                        $this->logger->log("error", "Time: {$time} | ID: $id | Target: Server | Status: 400 | Failed to generate file: /img/tmp/manufacturer_$id.jpg");
                    }

                    //generate tmp mini images
                    $imgThumbMini = ImageManager::thumbnail(
                        $mainImg,
                        "manufacturer_mini_$id.jpg",
                    ...
                    if (!empty($imgThumbMini)) {
                        $this->logger->log("generate", "Time: {$time} | ID: $id | Target: Server | Status: 200 | Image: /img/tmp/manufacturer_mini_$id.jpg");
                    } else {
                        $this->logger->log("error", "Time: {$time} | ID: $id | Target: Server | Status: 400 | Failed to generate file: /img/tmp/manufacturer_mini_$id.jpg");
                    }
                }
            }
        } else {
